...
function TransferFunctionTexture()
{
    ...
    {
 //     var color = KVS.RedColorMap( 0, 255, i );
        var alpha = i / 255.0;
        data[ 4 * i + 0 ] = Math.max( Math.cos( ( alpha - 0.5 ) * Math.PI ), 0.0 );
        data[ 4 * i + 1 ] = Math.max( Math.cos( ( alpha - 1.0 ) * Math.PI ), 0.0 );
        data[ 4 * i + 2 ] = Math.max( Math.cos( alpha * Math.PI ), 0.0 );
        data[ 4 * i + 3 ] = alpha;
    }
